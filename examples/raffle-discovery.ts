...
async function raffleDiscoveryExample() {
  ...
  try {
    ...
    allRaffles.raffles.forEach((raffle, index) => {
      ...
      console.log(
        `    Status: ${DripsUtils.getRaffleStatusDescription(raffle.status)}`
      );
    ...
    activeRaffles.raffles.forEach((raffle, index) => {
      console.log(
        `    ${index + 1}. ${raffle.nftMetadata?.name || "Unknown NFT"} - ${
          raffle.participantsCount
        } participants`
      );
      console.log(
        `       Time remaining: ${DripsUtils.getTimeRemaining(
          raffle.formattedDeadline
        )}`
      );
    });
    console.log();
    ...
    console.log(
      `  Found ${searchResults.raffles.length} raffles matching 'NFT'`
    );
    searchResults.raffles.forEach((raffle, index) => {
      console.log(`    ${index + 1}. ${raffle.nftMetadata?.name || "Unknown"}`);
      console.log(
        `       Description: ${
          raffle.nftMetadata?.description || "No description"
        }`
      );
    });
    console.log();
    ...
    const testCreatorAddress =
      "0x742d35cc6448c3d41b0e37b36b5e2e39db3b9d7b8b4d4c4e4f4a4b4c4d4e4f4a";
    ...
    try {
      ...
      if (creatorRaffles.raffles.length > 0) {
        console.log(
          `  Found ${creatorRaffles.raffles.length} raffles by creator`
        );
        ...
        console.log(
          "  No raffles found for this creator (this is expected for the test address)"
        );
      }
    } catch (error) {
      console.log(
        "  No raffles found for this creator (this is normal for test addresses)"
      );
    }
    console.log();
    ...
    const activeCount = allRaffles.raffles.filter(
      (r) => r.status.isActive
    ).length;
    const endedCount = allRaffles.raffles.filter(
      (r) => r.status.isEnded
    ).length;
    const withWinnerCount = allRaffles.raffles.filter(
      (r) => r.status.hasWinner
    ).length;
    ...
    const sortedByParticipants = [...allRaffles.raffles].sort(
    ...
    if (sortedByParticipants.length > 0) {
      console.log(
        `  Most popular: ${
          sortedByParticipants[0].nftMetadata?.name || "Unknown"
        } with ${sortedByParticipants[0].participantsCount} participants`
      );
    }

    console.log("\n✅ Raffle discovery complete!");
    console.log(
      "💡 You can now use these raffle IDs to join, check details, or manage raffles"
    );
  } catch (error) {
    console.error(
      "❌ Error during raffle discovery:",
